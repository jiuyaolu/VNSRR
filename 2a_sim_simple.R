  ...
  for (rp in c(0.5)) {
    for (r in c(0.5)) {
      for (S in c(1000,10000)) {
        ...
        p = round(n * rp)
        n1 = round(n * r)
        load(sprintf("data/sim_simple/sim_simple_n%s_p%s.RData",n,p))
...
if (test) {
  method_list = c("PSRR", "VNSRR")
} else if (n==500 & S==10000) {
  method_list = c("PSRR","VNSRR")
} else {
  method_list = c("AJSRR","PSRR","VNSRR")
}
met_tab = bind_rows(
  expand_grid(
    S = 1,
...
met_fun = function(i_met, i_rep) {
  if (met_tab$method[i_met] == "VNSRR") {
    time = system.time(
      design <- VNSRR(X[[i_rep]], n1 = n1, n_fisher = n_fisher, p_a = 1 / met_tab$S[i_met],
                      max_iter = 10 * met_tab$S[i_met])
    )[3] %>% unname()
  } else {
    pair_switch = met_tab$method[i_met] == "PSRR"
    time = system.time(
      design <- rerand(X[[i_rep]], n1 = n1, n_fisher = n_fisher, p_a = 1 / met_tab$S[i_met], 
                       pair_switch = pair_switch, max_iter = 10 * met_tab$S[i_met])
    )[3] %>% unname()
  }
...
if (!test) {
  save(design_met_rep_fisher, destime_met_rep_fisher,
       file = sprintf("output/sim_simple/sim_simple_n%s_p%s_nt%s_S%s_design_%s.Rdata",
...
if (!test) {
  save(infer_R2_met_rep, inftime_R2_met_rep,
       file = sprintf("output/sim_simple/sim_simple_n%s_p%s_nt%s_S%s_inference_%s.RData",
                      n, p, n1, S, ifelse(n==500&S==10000,"noAJS","all")))
}
}
...
if (!test) {
  if (run_inference) {
    save(infer_sum_tab, time_sum_tab,
         file = sprintf("output/sim_simple/sim_simple_n%s_p%s_nt%s_S%s_summary_%s.RData",
                        n, p, n1, S, ifelse(n==500&S==10000,"noAJS","all")))
  } else {
    save(time_sum_tab,
         file = sprintf("output/sim_simple/sim_simple_n%s_p%s_nt%s_S%s_summary_%s.RData",
