...
StrataAJSRR_one <- function(X, n_seq, n1_seq, prop_cov_inv, a, max_iter) {
  nstrata = length(n_seq)
  n_cum = c(0, cumsum(n_seq)[-nstrata])
  id_strata = lapply(1:nstrata, function(k) 1:n_seq[k]+n_cum[k])
  
  # start point
  Wt <- lapply(1:nstrata, function(k) sample(id_strata[[k]], n1_seq[k])) %>% unlist
  if (a == Inf) {
    cont <- FALSE
    Mt <- NA
    Wbest <- Wt
...
StrataAJSRR <- function(X, n_seq, n1_seq = round(n_seq / 2), n_fisher = 1, 
  ...
  if (parallel) {
    out <- mclapply(1:n_fisher, function(x) {
      StrataAJSRR_one(X, n_seq, n1_seq, prop_cov_inv, a, max_iter)
    }, mc.cores = n_cores)
  } else {
