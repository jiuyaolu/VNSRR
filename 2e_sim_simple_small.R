      ...
      for (S in c(1000, 10000)) {
        ...
        if (!test) {
          save(design_met_rep_fisher, destime_met_rep_fisher,
               file = sprintf("output/sim_simple_small/sim_simple_small_n%s_p%s_nt%s_S%s_design_%s.Rdata",
        ...
        if (run_inference) {
          ...
          if (!test) {
            save(infer_R2_met_rep, inftime_R2_met_rep,
                 file = sprintf("output/sim_simple_small/sim_simple_small_n%s_p%s_nt%s_S%s_inference_%s.RData",
                                n, p, n1, S, ifelse(n==500&S==10000,"all","all")))
          }
        }
        ...
        if (!test) {
          if (run_inference) {
            save(infer_sum_tab, time_sum_tab,
                 file = sprintf("output/sim_simple_small/sim_simple_small_n%s_p%s_nt%s_S%s_summary_%s.RData",
                                n, p, n1, S, ifelse(n==500&S==10000,"all","all")))
          } else {
            save(time_sum_tab,
                 file = sprintf("output/sim_simple_small/sim_simple_small_n%s_p%s_nt%s_S%s_summary_%s.RData",
