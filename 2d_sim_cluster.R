...
library(tictoc)
library(parallel)
source("rerand CR AJSRR cluster.R")
source("rerand VNSRR cluster.R")
source("fisher.R")
...
for (clust_size in c(2)) {
  for (p in c(50, 250)) {
    for (S in c(1000)) {
      ...
      n_clust = p * 2
      n1_clust = p
      load(sprintf("data/sim_cluster/sim_cluster_n%s_p%s.RData",n,p))
      prop_cov_inv <- lapply(1:n_rep, function(i) {
        n1 * (1 - n1 / n) * solve(cov(X[[i]]))
      })
      
      print(sprintf("n=%s clust_size=%s p=%s S=%s",n,clust_size,p,S))
...
if (test) {
  # method_list = c("ClustVNSRR")
  method_list = c("ClustAJSRR","ClustVNSRR")
} else {
  method_list = c("ClustAJSRR","ClustVNSRR")
}
...
met_fun <- function(i_met, i_rep) {
  if (met_tab$method[i_met] == "ClustVNSRR") {
    time <- system.time(
      design <- ClustVNSRR(X[[i_rep]], n_clust, n1_clust, n_fisher = n_fisher, 
                         p_a = 1 / met_tab$S[[i_met]],
                         max_iter = 10 * met_tab$S[[i_met]])
    )[3] %>% unname()
  } else {
    time <- system.time(
      design <- ClustAJSRR(X[[i_rep]], n_clust, n1_clust, n_fisher = n_fisher, 
                           p_a = 1 / met_tab$S[[i_met]],
                           max_iter = 10 * met_tab$S[[i_met]])
    )[3] %>% unname()
  }
...
if (!test) {
  save(design_met_rep_fisher, destime_met_rep_fisher,
       file = sprintf("output/sim_cluster/sim_cluster_n%s_p%s_nt%s_S%s_design_%s.Rdata",
                      n, p, n1, S, "all"))
}
...
if (!test) {
  save(infer_R2_met_rep, inftime_R2_met_rep,
       file = sprintf("output/sim_cluster/sim_cluster_n%s_p%s_nt%s_S%s_inference_%s.RData",
                      n, p, n1, S, "all"))
}
}
...
if (!test) {
  if (run_inference) {
    save(infer_sum_tab, time_sum_tab,
         file = sprintf("output/sim_cluster/sim_cluster_n%s_p%s_nt%s_S%s_summary_%s.RData",
                        n, p, n1, S, "all"))
  } else {
    save(time_sum_tab,
         file = sprintf("output/sim_cluster/sim_cluster_n%s_p%s_nt%s_S%s_summary_%s.RData",
