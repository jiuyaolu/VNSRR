...
library(tictoc)
library(parallel)
source("rerand CR AJSRR stratified.R")
source("rerand VNSRR stratified.R")
source("fisher.R")
...
for (nstrata in c(2)) {
  for (p in c(50, 250)) {
    for (S in c(1000)) {
      ...
      n = nstrata * p * 2
      n1 = round(n / 2)
      n_seq = rep(p*2, nstrata)
      n1_seq = rep(p, nstrata)
      load(sprintf("data/sim_stratified/sim_stratified_n%s_p%s.RData",n,p))
      prop_cov_inv <- lapply(1:n_rep, function(i) {
        n1 * (1 - n1 / n) * solve(cov(X[[i]]))
      })
      
      print(sprintf("n=%s nstrata=%s p=%s S=%s",n,nstrata,p,S))
...
if (test) {
  # method_list = c("StratVNSRR")
  method_list = c("StratAJSRR","StratVNSRR")
} else {
  method_list = c("StratAJSRR","StratVNSRR")
}
...
met_fun <- function(i_met, i_rep) {
  if (met_tab$method[i_met] == "StratVNSRR") {
    time <- system.time(
      design <- StratVNSRR(X[[i_rep]], n_seq, n1_seq, n_fisher = n_fisher, 
                         p_a = 1 / met_tab$S[[i_met]],
                         max_iter = 10 * met_tab$S[[i_met]])
    )[3] %>% unname()
  } else {
    time <- system.time(
      design <- StrataAJSRR(X[[i_rep]], n_seq, n1_seq, n_fisher = n_fisher, 
                           p_a = 1 / met_tab$S[[i_met]],
                           max_iter = 10 * met_tab$S[[i_met]])
    )[3] %>% unname()
  }
...
if (!test) {
  save(design_met_rep_fisher, destime_met_rep_fisher,
       file = sprintf("output/sim_stratified/sim_stratified_n%s_p%s_nt%s_S%s_design_%s.Rdata",
                      n, p, n1, S, "all"))
}
...
if (!test) {
  save(infer_R2_met_rep, inftime_R2_met_rep,
       file = sprintf("output/sim_stratified/sim_stratified_n%s_p%s_nt%s_S%s_inference_%s.RData",
                      n, p, n1, S, "all"))
}
}
...
if (!test) {
  if (run_inference) {
    save(infer_sum_tab, time_sum_tab,
         file = sprintf("output/sim_stratified/sim_stratified_n%s_p%s_nt%s_S%s_summary_%s.RData",
                        n, p, n1, S, "all"))
  } else {
    save(time_sum_tab,
         file = sprintf("output/sim_stratified/sim_stratified_n%s_p%s_nt%s_S%s_summary_%s.RData",
